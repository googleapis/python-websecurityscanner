...
from google.cloud.websecurityscanner_v1.services.web_security_scanner import (
...
from google.cloud.websecurityscanner_v1.types import scan_config as gcwv_scan_config
from google.cloud.websecurityscanner_v1.types import scan_config_error
from google.cloud.websecurityscanner_v1.types import (
    scan_config_error as gcwv_scan_config_error,
)
...
def test_create_scan_config(transport: str = "grpc"):
    ...
    with mock.patch.object(
        ...
        call.return_value = gcwv_scan_config.ScanConfig(
            name="name_value",
            display_name="display_name_value",
            max_qps=761,
            starting_urls=["starting_urls_value"],
            user_agent=gcwv_scan_config.ScanConfig.UserAgent.CHROME_LINUX,
            blacklist_patterns=["blacklist_patterns_value"],
            target_platforms=[gcwv_scan_config.ScanConfig.TargetPlatform.APP_ENGINE],
            export_to_security_command_center=gcwv_scan_config.ScanConfig.ExportToSecurityCommandCenter.ENABLED,
            risk_level=gcwv_scan_config.ScanConfig.RiskLevel.NORMAL,
        )
    ...
    assert isinstance(response, gcwv_scan_config.ScanConfig)
    assert response.name == "name_value"
    assert response.display_name == "display_name_value"
    assert response.max_qps == 761
    assert response.starting_urls == ["starting_urls_value"]
    assert response.user_agent == gcwv_scan_config.ScanConfig.UserAgent.CHROME_LINUX
    assert response.blacklist_patterns == ["blacklist_patterns_value"]
    assert response.target_platforms == [
        gcwv_scan_config.ScanConfig.TargetPlatform.APP_ENGINE
    ]
    assert (
        response.export_to_security_command_center
        == gcwv_scan_config.ScanConfig.ExportToSecurityCommandCenter.ENABLED
    )
    assert response.risk_level == gcwv_scan_config.ScanConfig.RiskLevel.NORMAL
...
def test_create_scan_config_flattened():
    ...
    with mock.patch.object(
        ...
        client.create_scan_config(
            parent="parent_value",
            scan_config=gcwv_scan_config.ScanConfig(name="name_value"),
        )
        ...
        assert args[0].scan_config == gcwv_scan_config.ScanConfig(name="name_value")
...
def test_list_scan_configs_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in client.list_scan_configs(request={},)]
...
def test_list_scan_runs_pager():
    ...
    with mock.patch.object(type(client._transport.list_scan_runs), "__call__") as call:
        ...
        results = [i for i in client.list_scan_runs(request={},)]
        assert len(results) == 6
...
def test_list_crawled_urls_pager():
    ...
    with mock.patch.object(
        ...
        results = [i for i in client.list_crawled_urls(request={},)]
        assert len(results) == 6
...
def test_list_findings_pager():
    ...
    with mock.patch.object(type(client._transport.list_findings), "__call__") as call:
        ...
        results = [i for i in client.list_findings(request={},)]
        assert len(results) == 6
...
def test_web_security_scanner_transport_auth_adc():
    ...
    with mock.patch.object(auth, "default") as adc:
        adc.return_value = (credentials.AnonymousCredentials(), None)
        transports.WebSecurityScannerGrpcTransport(host="squid.clam.whelk")
        adc.assert_called_once_with(
            scopes=("https://www.googleapis.com/auth/cloud-platform",)
        )
